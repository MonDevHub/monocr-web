	...
	<main id="main-content" class="space-y-8">
		...
		{:else}
			<div class="grid items-start gap-8 md:grid-cols-2" in:fly={{ y: 20, duration: 400 }}>
				...
				<div class="space-y-4">
					<div
						class="border-border bg-canvas-subtle relative overflow-hidden rounded-xl border shadow-sm"
					>
						<img
							src={previewUrl}
							alt="Preview"
					...
					<button
						on:click={reset}
						class="hover:text-fg-primary hover:decoration-fg-secondary text-fg-secondary decoration-border text-sm underline underline-offset-4 transition-colors"
					>
				...
				<!-- Result -->
				<div
					class="border-border bg-canvas flex h-full min-h-[300px] flex-col overflow-hidden rounded-xl border shadow-sm"
				>
					<div
						class="border-border bg-canvas-subtle flex items-center justify-between border-b px-6 py-4"
					>
						<h2 class="text-fg-primary font-semibold">Extracted Text</h2>
						{#if processingTime > 0 && !loading}
							<span class="text-fg-secondary font-mono text-xs">{processingTime}ms</span>
						{/if}
					</div>
					...
					<div class="relative flex-1 p-6">
						...
						{#if resultText}
							<div class="prose max-w-none dark:prose-invert">
								<p class="font-mon text-fg-primary text-lg leading-relaxed whitespace-pre-wrap">
									{resultText}
								</p>
							</div>
						{:else if !loading}
							<div class="text-fg-secondary flex h-full items-center justify-center italic">
								No text extracted
							</div>
						{/if}
					</div>
					...
					{#if resultText}
						<div class="border-border bg-canvas-subtle flex justify-end border-t px-4 py-3">
							<button
